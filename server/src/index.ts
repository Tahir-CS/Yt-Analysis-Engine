...
import { generateCreatorReportPDF } from './services/reportGenerator';
...
server.get('/api/v1/channel/lookup', async (request: FastifyRequest<{
  Querystring: { handle?: string; q?: string };
}>, reply: FastifyReply) => {
...
server.post('/api/v1/predict', async (request: FastifyRequest<{
  Body: {
    points: DataPoint[];
    baseline?: CreatorHistoricalBaseline;
    videoTitle?: string;
  };
}>, reply: FastifyReply) => {
  try {
    ...
    const fitResult = fitVideoViewsCurve(points);

    let aiCheck = null;
    if (baseline && videoTitle && process.env.GEMINI_API_KEY) {
      aiCheck = await sanityCheckPredictionWithAI(fitResult.vMax, baseline, videoTitle);
    }
...
server.post('/api/v1/valuation', async (request: FastifyRequest<{
  Body: ValuationMetrics;
}>, reply: FastifyReply) => {
  try {
    ...
    const valuation = await calculateSponsorshipValuation({
...
server.get('/api/v1/fyp-radar', async (_request: FastifyRequest, reply: FastifyReply) => {
...
server.post('/api/v1/fyp-radar/hook-analysis', async (request: FastifyRequest<{
  Body: { title: string; hookScript?: string };
}>, reply: FastifyReply) => {
  ...
  if (hasNumbers) viralProbability += 12;
  if (hasEmotionalTrigger) viralProbability += 15;
  if (length >= 35 && length <= 65) viralProbability += 10;
  if (hasQuestion) viralProbability += 5;
...
server.post('/api/v1/reports/pdf', async (request: FastifyRequest<{
  Body: { creatorId: number; htmlContent?: string };
}>, reply: FastifyReply) => {
  try {
    const { creatorId, htmlContent } = request.body || {};
    const pdfBuffer = await generateCreatorReportPDF(creatorId || 1, htmlContent);

 ...
 * ============================================================================
 */
export async function analyzeAndStoreCommentSentiment(
  creatorId: number,
  videoId: string,
  commentText: string
) {
  try {
    ...
    await query(
      `INSERT INTO comment_sentiments (creator_id, video_id, comment_text, sentiment_score, embedding)
       VALUES ($1, $2, $3, $4, $5)`,
      [creatorId, videoId, commentText, sentimentScore, JSON.stringify(embeddingVector)]
    );
 ...
 */
export async function findSimilarComments(commentText: string, limit: number = 5) {
  const embeddingResponse = await ai.models.embedContent({
    model: 'text-embedding-004',
    contents: commentText,
  });
  ...
  const res = await query(
    `SELECT comment_text, sentiment_score, 1 - (embedding <=> $1) as similarity
     FROM comment_sentiments
     ORDER BY embedding <=> $1 ASC
     LIMIT $2`,
    [JSON.stringify(embeddingVector), limit]
  );

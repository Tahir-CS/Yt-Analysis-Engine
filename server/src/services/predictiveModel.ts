...
export interface DataPoint {
  t: number; // Hours since upload (e.g. 1, 2, 3, 4, 5, 6)
  v: number; // Cumulative views at hour t
}
 ...
 */
export function fitVideoViewsCurve(points: DataPoint[]): ModelFitResult {
  if (points.length < 3) {
...
export interface CreatorHistoricalBaseline {
  averageLifetimeViews: number;
  highestLifetimeViews: number;
  lowestLifetimeViews: number;
  niche: string;
}

export async function sanityCheckPredictionWithAI(
